        ...
        <div>
          {searchResults && searchResults.length > 0 ? (
            searchResults.map((post, index) => {
              ...
              return (
                <div className="relative flex flex-col mt-20 text-gray-700 bg-white shadow-md bg-clip-border rounded-sm xl:w-[38rem] lg:w-[30rem] md:w-[26rem] sm:w-[22rem] w-full" key={index} style={postCardStyle}>
                  <div className='flex items-center p-2'>
                    {post.channel?.image ? (
                      <img className="w-8 h-8 rounded-full" src={post.channel?.image} alt="channel"/>
                    ) : (
                  ...
                  <GetComments postId={post?._id} likeCount={post?.likeCount} commentCount={post?.commentCount} postTitle={post?.title} postContent={post?.content} />
                </div>
              );
            })
          ) : (
